...
api = tweepy.API(auth, wait_on_rate_limit=True, retry_count=3, retry_delay=120, retry_errors=set([503]))
...
for m in range (currentEPnum, allEPs_lines):
    while True:
        try:
            ...
            totalFrames2 = [os.path.join(frame_path, file) for file in os.listdir(frame_path) if file.endswith('.jpg')] 
            for i in range (currentFrame, len(totalFrames2), 4):
                while True:
                    try:
                        j = i + 1
                        k = i + 2
                        ...
                        image1_path = ("{frame_path}/frame-{i}.jpg".format(frame_path=frame_path, i=i))
                        image2_path = ("{frame_path}/frame-{j}.jpg".format(frame_path=frame_path, j=j))
                        image3_path = ("{frame_path}/frame-{k}.jpg".format(frame_path=frame_path, k=k))
                        image4_path = ("{frame_path}/frame-{l}.jpg".format(frame_path=frame_path, l=l))
                        
                        #Uploading images of frames to Twitter and get media IDs
                        media_ids = []
                        
                        # 1st frame
                        print("Uploading image of frame {i} to Twitter".format(i=i))
                        media = api.media_upload(image1_path)
                        sleep(5)
                        media_ids.append(media.media_id)
                        # 2nd frame
                        check_image2 = os.path.isfile("{frame_path}/frame-{j}.jpg".format(frame_path=frame_path, j=j))
                        if check_image2:
                            print("Uploading image of frame {j} to Twitter".format(j=j))
                            media = api.media_upload(image2_path)
                            sleep(5)
                            media_ids.append(media.media_id)
                        # 3rd frame
                        check_image3 = os.path.isfile("{frame_path}/frame-{k}.jpg".format(frame_path=frame_path, k=k))
                        if check_image3:
                            print("Uploading image of frame {k} to Twitter".format(k=k))
                            media = api.media_upload(image3_path)
                            sleep(5)
                            media_ids.append(media.media_id)
                        # 4th frame
                        check_image4 = os.path.isfile("{frame_path}/frame-{l}.jpg".format(frame_path=frame_path, l=l))
                        if check_image4:
                            print("Uploading image of frame {l} to Twitter".format(l=l))
                            media = api.media_upload(image4_path)
                            sleep(5)
                            media_ids.append(media.media_id)
                            framestext = "Frames {i}, {j}, {k}, {l}".format(i=i, j=j, k=k, l=l)
                        
                        if not check_image4:
                            framestext = "Frames {i}, {j}, {k}".format(i=i, j=j, k=k)
                        if not check_image3:
                            framestext = "Frames {i}, {j}".format(i=i, j=j)
                        if not check_image2:
                            framestext = "Frame {i}".format(i=i)
                        ...
                        if enableEPname == 1:
                            ...
                            if EPname == "":
                                full_tweet = '{showName} - {season_episode} - {framestext} out of {totalFrames}'.format(showName=showName, season_episode=season_episode, framestext=framestext, totalFrames=totalFrames)
                            else:
                                full_tweet = '{showName} - {season_episode} "{EPname}" - {framestext} out of {totalFrames}'.format(showName=showName, season_episode=season_episode, EPname=EPname, framestext=framestext, totalFrames=totalFrames)
                        else:
                            full_tweet = '{showName} - {season_episode} - {framestext} out of {totalFrames}'.format(showName=showName, season_episode=season_episode, framestext=framestext, totalFrames=totalFrames)
                        
                        #Send tweet with uploaded image and text
                        print("Sending tweet with frames {i}, {j}, {k}, {l} out of {totalFrames}".format(i=i, j=j, k=k, l=l, totalFrames=totalFrames))
                        logging.info("Sending tweet with frames {i}, {j}, {k}, {l} out of {totalFrames}".format(i=i, j=j, k=k, l=l, totalFrames=totalFrames))
                        client.create_tweet(text=full_tweet, media_ids=media_ids)
